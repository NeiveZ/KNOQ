...
${BOLD}KNOQ - Knock Network Open Query${R}
...
if [[ -n "$OUTPUT" ]]; then
    printf "# KNOQ | target: %s | knock: %s | port: %s | %s\n\n" \
...
if [[ "$MODE" == "single" ]]; then
    ...
    log info "knoq | single host | knock: ${KNOCK_PORTS} | target port: ${TARGET_PORT}"
    echo
    scan_host "$TARGET"

else

    log info "knoq | subnet ${TARGET}.0/24 | knock: ${KNOCK_PORTS} | target port: ${TARGET_PORT} | threads: ${THREADS}"
    echo
...
[[ -n "$OUTPUT" ]] && log info "Saved: ${OUTPUT}"
